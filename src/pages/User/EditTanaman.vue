  ...
  <q-page padding class="items-center-justify-center bg-grey-3">
    <q-card flat class="bg-white q-pa-md">
      <span class="text-h5 text-weight-light q-pa-md">
        <span class="text-blue-grey-14">Ubah Data Tanaman</span>
      </span>
      <div class="q-pa-md">
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      api.put(`tanaman/edit/${this.$route.params.id}`, formData).then((res) => {
        console.log(res);
        if (res.data.status) {
          this.$q.notify({
            color: "positive",
    ...
    getTanamanById() {
      api.get(`tanaman/get/${this.$route.params.id}`).then((res) => {
        console.log(res);
